...
def extract_keywords(document):
    #jieba
    keywords = jieba.analyse.textrank(document, topK=5)
    return keywords
...
def textrank_scoring(similarity_matrix, d=0.85, max_iter=100, tol=1e-3):
    # 使用TextRank算法计算句子分数
    num_sentences = similarity_matrix.shape[0]

    scores = np.ones(num_sentences) / num_sentences # 初始化分数
    ...
    for _ in range(max_iter):
        ...
        for i in range(num_sentences):
            # 计算公式中的 ∑j∈In(Vi)∣Out(Vj)∣1​S(Vj) 部分，使用相似度矩阵替代
            denominator = np.sum((similarity_matrix[i] / np.sum(similarity_matrix, axis=1)) * scores)
            scores[i] = (1 - d) + d * denominator
...
def textrank_bert(text, model,num_sentences=-1, d=0.85):
    # 加载预训练的BERT模型和分词器
    ...
    # 使用独立的函数计算句子分数
    scores = textrank_scoring(similarity_matrix, d=d)

    # 根据分数排序，提取前 num_sentences 个句子
    ranked_sentences_indices = np.argsort(scores)[::-1][:num_sentences]
    selected_sentences = [sentences[i] for i in ranked_sentences_indices]

    return selected_sentences, scores
...
if __name__ == "__main__":
    ...
    # 调用 textrank_bert 函数进行关键句提取
    # result, scores = textrank_bert(text,model,num_sentences=20)
    print(extract_keywords(text))
    print(textrank_extract_keywords(text))
    print(top_k_sentences(text))
    # 打印结果和权重
    # for i, (sentence, score) in enumerate(zip(result, scores), 1):
    #     print(f"{i}. {sentence} (Weight: {score:.4f})")
